...
## v65.2 공개/잠금 토글 수정
- 일반 주소는 로그인 세션이 아니라 각 페이지의 `페이지 상태`를 따릅니다.
- `잠금`이면 일반 방문자는 데이터 자체를 읽지 못합니다.
- `공개`이면 해당 페이지에서 상태가 `공개`인 문서만 읽습니다.
- 인물 프로필은 인물 페이지가 공개일 때만 읽을 수 있습니다.
- 이 버전의 `firestore.rules`를 Firebase 콘솔에 함께 게시해야 일반 공개가 동작합니다.


## v65.3
- STORY 비주얼 노벨에서 대사창 클릭 이벤트가 스테이지와 중복 실행되어 한 번 클릭에 두 장면씩 넘어가던 문제 수정.
- 스테이지 클릭은 한 번에 정확히 한 장면만 진행.
